...
function CamposValidos(){
    ...
    if (nombre == "") 	{
        mostrarMensaje("Debe indicar Nombre",MSG_STOP);
        return false;
        }
    return true;
    }; 
        ...
        buttons: [
            {
                ...
                click: function() {
                    $( this ).dialog("close");
                    confirmarMensaje("The register will be removed from the Database.",EnviaPeticionAjax,ELIMINAR_REG,id);
                    },
...
function EnviaPeticionAjax(accion,id){
    var nombre = allTrim($('#nombre').val());
    var tabla = allTrim($('#tabla').val());
    var miurl = '/tablas/tablasimple/'+tabla+'/';
    ...
    try {
        $.ajax({
			...
			dataType: "html",
            url: miurl,
            data: {accion: accion, id: id, nombre: nombre, tabla: tabla, csrfmiddlewaretoken: $("input[name=csrfmiddlewaretoken]").val()},
